...
def selNSGA2R(individuals, k):
    ...
    pareto_fronts, ranks = sortFitnessNonDominated(fitnesses, k)
...
def sortFitnessNonDominated(fitnesses, k, first_front_only=False):
...
def selTournamentFitnessDCD(individuals, k):
    ...
    while len(chosen) < k:
        samples = random.sample(fitnesses, min((k-len(chosen))*2, len(fitnesses)))
        for fit1, fit2 in zip(samples[::2], samples[1::2]):
            ind1 = random.choice(unique_fits[fit1])
            ind2 = random.choice(unique_fits[fit2])
...
__all__ = ['selNSGA2', 'selNSGA2R', 'selNSGA3', 'selNSGA3WithMemory', 'selSPEA2', 'sortNondominated', 'sortLogNondominated',
           'selTournamentDCD', 'selTournamentFitnessDCD', 'uniform_reference_points']
